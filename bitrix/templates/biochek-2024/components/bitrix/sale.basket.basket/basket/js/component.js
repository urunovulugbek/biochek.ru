;(function() {
	...
	BX.Sale.BasketComponent = {
		...
		fireCustomEvents: function()
		{
			...
			BX.onCustomEvent('OnCouponChange');
		},
		...
		bindTotalEvents: function(node)
		{
			...
			BX.bind(this.getEntity(node, 'basket-coupon-add'), 'click', BX.proxy(this.addCouponAction, this));
		...
		{
			var target = this.getEntity(this.getCacheNode(this.ids.basketRoot), 'basket-coupon-input');

			//var target = BX.getEventTarget(event);
			if (target && target.value)
			{
				var value = target.value;
				this.actionPool.addCoupon(value);
				target.disabled = true;
				this.onCouponChanged(function() {
					if (!/_[^_]+_/.test(value)) {
						this.actionPool.addCoupon('_' + value + '_');
					}
				}.bind(this));
			}
		},
		...
		onCouponChanged: function(callback)
		{
			var callbackWrap = function() {
				callback();
				BX.removeCustomEvent('OnCouponChange', callbackWrap);
			}
			BX.addCustomEvent('OnCouponChange', callbackWrap);
		},
		...
		removeCouponAction: function()
		{
			...
			if (value)
			{
				var extCoupon = '_'+value+'_';
				var haveExtCoupon = false;
				for (var i = 0; i < this.coupons.length; i++) {
					if (this.coupons[i].COUPON == extCoupon) {
						haveExtCoupon = true;
						break;
					}
				}
				...
				if (haveExtCoupon) {
					this.actionPool.removeCoupon(extCoupon);
					this.onCouponChanged(function() {
...
$(document).ready(function () {
	$('.basket-form_plan-switch input').on('change', function() {
		if ($(this).prop('checked')) {
			$('.basket-form_btn').addClass('active');
		} else {
			$('.basket-form_btn').removeClass('active');
		}
	});
});